...
xepOnline.Formatter = {
	...
	setSVGHeightWidth: function(dest) {
	    $(dest).find('svg').each(function(index) {
			var svg = $(this);
	...
	replaceCanvas: function(dest) {
	    $(dest).find('canvas').each(function(index) {
			var canvas = this;
			var src_canvas = $($(xepOnline.Formatter.__elm)[0]).find('canvas')[index];
			$('<img src="' + src_canvas.toDataURL() +'"/>').insertAfter(canvas);
		});
	},
    handlePseudoElem: function(dest) {
	    $(dest).find('*').each(function(index) {
			var elem = this;
			...
			if(before.getPropertyValue('content').length > 0 && before.getPropertyValue('content') != "none"){
			     ...
			     var parentStyle = xepOnline.Formatter.getRealStyle($(elem).parent()[0], xepOnline.Formatter.fo_attributes, null);
                 ...
                 before_text.forEach(function (part){
                     ...
                     if (part.indexOf("ttp://") >= 0 && in_image == true) {
                         var img = $('<img src="url(&quot;'+ part + '&quot;)">');
			             $(span_before).append(img);
			             processed = true;
                     }
                     ...
                     if (processed == false) {
                        var textspan = $('<span>');
                        $(textspan).text(part);
                        $(span_before).append(textspan);
                     }
                     processed = false;
                 }); 
                 $(elem).prepend(span_before);
            }
			...
			if(after.getPropertyValue('content').length > 0 && after.getPropertyValue('content') != "none"){
                 ...
                 after_text.forEach(function (part){
                     ...
                     if (processed == false) {
                        var textspan = $('<span>');
                        $(textspan).text(part);
                        $(span_after).append(textspan);
                     }
                     processed = false;
                 }); 
                 $(elem).append(span_after);
            }
		});
	},
	embedLocalImages: function(dest) {
	    $(dest).find('img').each(function(index) {
			var img = this;
	...
	computeTableCols: function(dest) {
		$(dest).find('table').each(function() {
			var table = this;
			$(table).find('col,colgroup').each(function() {
				$(this).attr('xeponline-drop-me',true);
			});

				var cols = 0;
				$($.find('dest td,th',$('tr',table)[0])).each(function(td) { cols += Number((Number($(td).attr('colspan'))) ? ($(td).attr('colspan')): 1); })
				var tbody = $('<tbody>');
				var tr = $('<tr>');
				$(tbody).append(tr);

				for(var i = 0; i<cols; i++) {
					$(tr).append('<td style="padding:0; margin:0">&#x200b;</td>');
				}

				// append tbody after everything else
				$(table).append(tbody);
				var widths = [];
				$($($('tr',tbody)[0])).find('td,th').each(function() { 
					widths.push($(this).css('width').replace('px',''));
				});

				// remove any original col groups and widths
				$(table).find('[xeponline-drop-me=true]').remove();
				$(table).find('td,th').css('width','');

				var colgroup = $('<colgroup>');
				$(table).prepend(colgroup);
				for(var i = 0; i<widths.length;i++) {
					var col = $('<col>');
					$(col).attr('width', widths[i]);
					$(colgroup).append(col);
				}
				$(tbody).remove();
		});
			...
			switch(elm2.tagName) {
				case 'IFRAME':
					try {
						...
						var content = $($(xepOnline.Formatter.__elm).find('iframe[src="' + $(elm2).attr('src') + '"]')[0].contentDocument).find('[contenteditable]');
						var iflat = $('<div data-xeponline-formatting=\'i-flat\'></div>');
				...
				break;
				default:
					var parentStyle = xepOnline.Formatter.getRealStyle($(elm2).parent()[0], xepOnline.Formatter.fo_attributes, null);
	...
	getFOContainer: function(elm, options) {
		...
		if (options.srctype == 'svg'){
		    var svgcontainer = $('<div class=\'svg-container\' width=' + $(elm).width() + ' height=' + $(elm).height() + '></div>');
		...
		if(options && options.cssStyle) {
		    $.each(options.cssStyle, function(key, value) {
	...
	*/
	__format: function(ElementIDs, options) {
		...
		current_height = $('#' + ElementIDs[0]).height();
		...
		$(ElementIDs).each(function(index, ElementID){
		   ...
		   xepOnline.Formatter.__container = xepOnline.Formatter.getFOContainer(xepOnline.Formatter.__elm, options);
			...
			$(xepOnline.Formatter.__clone).appendTo($(xepOnline.Formatter.__container).children(1));		

		   xepOnline.Formatter.togglePrintMediaStyle();
		   xepOnline.Formatter.flattenStyle($(xepOnline.Formatter.__container)[0], options);
		...
		var nss = "";
		$.each(options.namespaces || [], function(objkey, objvalue) {
                nss += objvalue + ' ';
        });
		...
		if(options.render === 'download') {
			...
			$('#temp_post').append('<input type=\'hidden\' name=\'mimetype\' value=\'' + options.mimeType + '\'/>');
			$('#temp_post').append('<input type=\'hidden\' name=\'filename\' value=\'' + options.filename + '\'/>');
			$('#temp_post').append('<input type=\'hidden\' name=\'xml\' value=\'' + xepOnline.Formatter.getFormTextData(printcopy) + '\'/>');
			$('#temp_post').submit();
			$('#temp_post').remove();
			$(document).trigger( "xepOnlineStatus", ["Finished"]);
...
'<SOAP-ENV:Envelope xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" \
	...
	Format: function(ElementID, options) {
	    $(document).trigger( "xepOnlineStatus", ["Started"]);
		var items;
	...
	__soapBackSuccess: function(xmlHttpRequest, status) {
		var base64PDF = $(xmlHttpRequest.responseXML).find('formatReturn').text();
		var objbuilder = '';
		...
		if($(xepOnline.Formatter.__container).attr('data-xeponline-embed-pending') === 'true') {			
			$(xepOnline.Formatter.__elm).html(objbuilder);
			$(xepOnline.Formatter.__elm).css({'height': xepOnline.DEFAULTS.pageHeight });
			$(xepOnline.Formatter.__container).remove();
		} else {
	...
	__postBackSuccess: function(Response) {
	    $(document).trigger( "xepOnlineStatus", ["Finished"]);
		var base64 = $(Response).find("Result").text();
		...
		if (current_mimetype == 'image/svg+xml'){
		  if($(xepOnline.Formatter.__container).attr('data-xeponline-embed-pending') === 'true'){
                ...
                $(xepOnline.Formatter.__elm).html('');
                document.getElementById($(xepOnline.Formatter.__elm).attr('id')).appendChild(dom.documentElement);
		  }
		  else{
		      var win = window.open("","_blank","titlebar=yes");
			  win.document.title = "XEPOnline Result";
			  win.document.write('<html><head>');
			  $.each($("link[href*='googleapis']"),function(index,value){
	...
	__processImages: function(Response) {
	    $(document).trigger( "xepOnlineStatus", ["Finished"]);
	    var Pages = $(Response).find("Pages").children();
	    
	     if($(xepOnline.Formatter.__container).attr('data-xeponline-embed-pending') === 'true'){
	         $(xepOnline.Formatter.__elm).html('');
             $.each(Pages,function(index,value){
			     $(xepOnline.Formatter.__elm).append('<img style="border:1px solid black" height="100%" src="data:image/png;base64,' + $(value).text() + '"/>');
			 });
	     }
	     else {
	            if($(xepOnline.Formatter.__container).attr('data-xeponline-download-pending') === 'true'){
	               $.each(Pages,function(index,value){
			          var blob = base64toBlob($(value).text(),current_mimetype,512);
			         ...
			         win.document.write('</head><body height="100%"><div id="target" style="height:100%;overflow-y:auto">');
			         $.each(Pages,function(index,value){
			             win.document.write('<img style="border:1px solid black" src="data:' + current_mimetype + ';base64,' + $(value).text() + '"/>');
			         });
			         win.document.write('</div></body></html>');	
	             }
	     }
